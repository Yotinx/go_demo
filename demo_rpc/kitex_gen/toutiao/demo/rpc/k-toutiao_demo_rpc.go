...
func (p *GetItemRequest) fastWriteField255(buf []byte, w thrift.NocopyWriter) int {
	offset := 0
	if p.IsSetBase() {
...
func (p *GetItemRequest) DeepCopy(s interface{}) error {
	src, ok := s.(*GetItemRequest)
	if !ok {
		return fmt.Errorf("%T's type not matched %T", s, p)
	}
...
func (p *GetItemResponse) DeepCopy(s interface{}) error {
	src, ok := s.(*GetItemResponse)
	if !ok {
		return fmt.Errorf("%T's type not matched %T", s, p)
	}
...
func (p *ItemServiceGetItemArgs) DeepCopy(s interface{}) error {
	src, ok := s.(*ItemServiceGetItemArgs)
	if !ok {
		return fmt.Errorf("%T's type not matched %T", s, p)
	}

	var _req *GetItemRequest
	if src.Req != nil {
		_req = &GetItemRequest{}
...
func (p *ItemServiceGetItemResult) DeepCopy(s interface{}) error {
	src, ok := s.(*ItemServiceGetItemResult)
	if !ok {
		return fmt.Errorf("%T's type not matched %T", s, p)
	}

	var _success *GetItemResponse
	if src.Success != nil {
		_success = &GetItemResponse{}
